...
|---|---:|---:|
| Unique transformer blocks | 12 | 3 |
| Stack repetitions | 1 | 4 |
| Effective block applications | 12 | 12 |
| Hidden width / attention heads | 512 / 8 | 512 / 8 |
...
Both runs use the same seeded batch generator, so every optimizer step receives identical sampled training windows, including after resume. Initialization shares token/position weights and the first three block values at the same seed; the remaining standard blocks are independently initialized. Each model gets its own optimizer. GPU kernels may not be bitwise deterministic. Run several seeds before interpreting differences as reliable findings. No separately tuned hyperparameters are claimed.
...
Training runs the standard model, then the looped model, on one visible GPU. The default processes **32,768,000 training tokens per model**. Data windows are sampled with replacement; this is a fixed-token-budget experiment, not an epoch-based traversal. Training and validation print JSON to stdout with immediate flushing.
...
CUDA_VISIBLE_DEVICES=0 ./scripts/train.sh --output runs/h100-seed42
...
Under `runs/h100/standard/` and `runs/h100/looped/`:
...
```bash
./scripts/evaluate.sh --checkpoint runs/h100/standard/best.pt
./scripts/evaluate.sh --checkpoint runs/h100/looped/best.pt
./scripts/infer.sh --checkpoint runs/h100/looped/best.pt --prompt 'The history of science'
# Greedy generation:
./scripts/infer.sh --checkpoint runs/h100/standard/best.pt --prompt 'The Earth' --temperature 0
...
`configs/smoke.json` is for CPU verification only. For research runs, copy `configs/h100.json`, modify `training.seed`, and give each paired run a separate output directory. Try seeds 42, 43 and 44. To change loop factor, retain `depth == loop_layers * loops`, for example 12 = 6×2, 3×4 or 1×12. Keeping 12×1 for the looped model gives the architecture-equivalence control. Each configuration has the same width in both arms; parameter matching is a different experiment.
