...
def prepare(output, vocab_size=8192, local_dir=None, dataset_config='wikitext-2-raw-v1'):
    output = Path(output)
    ...
    if local_dir:
        texts = {s: Path(local_dir, f'{s}.txt').read_text().splitlines() for s in ('train', 'validation', 'test')}
        ...
        texts = {s: ds[s]['text'] for s in ('train', 'validation', 'test')}
    ...
    for split, rows in texts.items():
        ...
        for row in rows:
            if row.strip():
                ids.extend(tokenizer.encode(row).ids)
                ids.append(tokenizer.token_to_id('<eos>'))
        np.asarray(ids, dtype=np.uint32).tofile(output / f'{split}.bin')
        manifest['splits'][split] = {'tokens': len(ids), 'sha256': digest(output / f'{split}.bin')}
